  ...
  >
    <div class="max-w-7xl mx-auto">
      ...
      <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
        ...
        <!-- Resumen de filtros -->
        <div
          v-if="filteredData.length < bienes.length"
          class="mt-3 text-sm text-gray-600"
        >
          Mostrando {{ filteredData.length }} de {{ bienes.length }} bienes
        </div>
      </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                  <span
                    :class="obtenerEstilosEstado(bien.estado_id).contenedor"
                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                  >
                    <span
                      :class="obtenerEstilosEstado(bien.estado_id).punto"
                      class="w-2 h-2 rounded-full mr-1.5"
                    ></span>
    ...
    <!-- Modales -->
    <modal-bien
      :isOpen="openModalBien"
      @close="openModalBien = false"
      @save="addBien"
    />
    <modal-detalle-bien
      v-if="bienSeleccionado"
    ...
    <bar-code-modal
      ...
      @close="openModalBarCode = false"
      @search="handleSearch"
    />

    <!-- Add this with your other modals -->
    <modal-movimientos
      :isOpen="openModalMovimientos"
      :bien="bienParaMovimientos"
      @close="openModalMovimientos = false"
...
const handleSearch = ({
  ...
  const bienEncontrado = bienes.value.find((b) => b.codigo === barcode);
  if (bienEncontrado) {
...
onMounted(async () => {
  try {
    ...
    console.log("🔄 Iniciando carga de bienes...");
    console.log(
      "🔑 Token actual:",
    ...
    if (response.success) {
      ...
      console.log("✅ Bienes cargados:", bienes.value.length);
    } else {
      error.value = response.message || "Error al cargar los datos.";
      console.error("❌ Error en response:", error.value);
    }
  } catch (e: any) {
    ...
    // ✅ AGREGAR ESTO PARA VER DEBUG INFO DEL SERVIDOR
    if (e.response?.data?.debug) {
      console.error("🐛 DEBUG INFO FROM SERVER:");
      e.response.data.debug.forEach((log: string) => {
        console.error("   ", log);
      });
    }

    // ✅ TAMBIÉN VER verifyResult SI EXISTE
    if (e.response?.data?.verifyResult) {
      console.error("🔍 VERIFY RESULT:", e.response.data.verifyResult);
    }

    error.value =
      e.response?.data?.message || "Error de conexión al cargar los datos.";

    // Si es error de autenticación, no redirigir automáticamente
...
const filteredData = computed(() =>
  bienes.value.filter((b) => {
    ...
    const matchesCategoria =
      !selectedCategoria.value || b.categoria === selectedCategoria.value;
    const matchesEstado =
      !selectedEstado.value || b.estado === selectedEstado.value;
    const matchesUbicacion =
      !selectedUbicacion.value || b.ubicacion === selectedUbicacion.value;

    return (
      matchesSearch && matchesCategoria && matchesEstado && matchesUbicacion
    );
  })
);
...
const editarBien = (bien: Bien) => {
  ...
  alert(`Editar bien ${bien.codigo}`);
  // Podrías abrir un modal con los datos del bien para editar
};

const eliminarBien = async (bien: Bien) => {
  if (!confirm(`¿Está seguro de eliminar el bien ${bien.codigo}?`)) return;

  try {
    const response = await bienService.delete(bien.id!);

    if (response.success) {
      // Remover del array local
      bienes.value = bienes.value.filter((b) => b.id !== bien.id);
      alert("Bien eliminado exitosamente");
    } else {
      alert(response.message || "Error al eliminar");
    }
  } catch (e: any) {
    alert(e.response?.data?.message || "Error al eliminar el bien");
    console.error(e);
  }
};
